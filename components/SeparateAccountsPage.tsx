...
const SeparateAccountsPage: React.FC<SeparateAccountsPageProps> = ({ cards, transactions, onAddTransactionClick, onEditTransaction, onDeleteTransaction, selectedCardId, setSelectedCardId }) => {
    ...
    const { cardStats, transactionsForMonth } = useMemo(() => {
        if (!selectedCardId) {
            return { cardStats: { totalIncome: 0, totalExpense: 0, cardBalance: 0 }, transactionsForMonth: [] };
        }
        ...
        const monthlyTransactions = transactions.filter(t => { // Use all transactions for list
        ...
        const monthlyTransactionsForCalc = monthlyTransactions.filter(t => !t.isFuture);

        const totalIncome = monthlyTransactionsForCalc.reduce((acc, t) => t.amount > 0 ? acc + t.amount : acc, 0);
        const totalExpense = monthlyTransactionsForCalc.reduce((acc, t) => t.amount < 0 ? acc + Math.abs(t.amount) : acc, 0);

        return {
            cardStats: { totalIncome, totalExpense, cardBalance },
    ...
    if (cards.length === 0) {
        return (
             <main className="mt-8 animate-fade-in">
                ...
                <div className="bg-brand-surface p-12 rounded-3xl border border-brand-surface-alt text-center">
                    <span className="material-symbols-outlined text-5xl text-brand-text-secondary mb-4">credit_card_off</span>
                    <h2 className="text-xl font-semibold mb-2">Keine separaten Konten</h2>
                    <p className="text-brand-text-secondary max-w-md mx-auto">
                        Konten, die Sie von der Gesamtbilanz ausschließen, werden hier angezeigt. Sie können dies in den Kontoeinstellungen ändern.
                    </p>
                </div>
            </main>
        );
                 ...
                 <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-brand-surface p-6 rounded-3xl border border-brand-surface-alt">
                         ...
                         <div className="bg-brand-surface p-6 rounded-3xl border border-brand-surface-alt">
                            <p className="text-lg font-semibold text-brand-text-secondary mb-2">Einnahmen ({currentDate.toLocaleString('de-DE', { month: 'long', timeZone: 'Europe/Vienna' })})</p>
                            <p className="text-4xl font-bold mt-2 text-brand-accent-green">
                                +€{cardStats.totalIncome.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </p>
                        </div>
                        <div className="bg-brand-surface p-6 rounded-3xl border border-brand-surface-alt">
                            <p className="text-lg font-semibold text-brand-text-secondary mb-2">Ausgaben ({currentDate.toLocaleString('de-DE', { month: 'long', timeZone: 'Europe/Vienna' })})</p>
                            <p className="text-4xl font-bold mt-2 text-brand-accent-red">
                    ...
                    <div className="bg-brand-surface p-6 rounded-3xl border border-brand-surface-alt">
                         <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-semibold">Transaktionen in {currentDate.toLocaleString('de-DE', { month: 'long', timeZone: 'Europe/Vienna' })}</h3>
                            <button 
                        ...
                        <div className="space-y-1">
                             {transactionsForMonth.length > 0 ? (
                                transactionsForMonth.map(t => (
                                     <div 
                                        key={t.id} 
                                        className={`grid grid-cols-2 md:grid-cols-4 gap-4 items-center p-3 -m-3 rounded-xl hover:bg-brand-surface-alt transition-all duration-200 ${t.isFuture ? 'opacity-50' : ''}`}
                                    >
